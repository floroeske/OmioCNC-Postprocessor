...
TOOLTIP='''
Generate g-code from a Path that is compatible with the omio controller.

import omio_post
omio_post.export(object,"/path/to/file.ncc")
'''
...
parser = argparse.ArgumentParser(prog='omio', add_help=False)
...
# Wait for spindle to start before doing first move
WAIT_FOR_SPINDLE = True
SPINDLE_WAIT_TIME = 10000 # Time in ms
SPINDLE_IS_STARTING = False
...
def parse(pathobj):
    ...
    global SUPPRESS_TOOL_CHANGE
    global WAIT_FOR_SPINDLE
    global SPINDLE_IS_STARTING
    ...
    if hasattr(pathobj,"Group"): #We have a compound or project.
        ...
        for c in pathobj.Path.Commands:
            ...
            if SPINDLE_IS_STARTING:
                out += linenumber() + "(wait for spindle)\n"
                out += linenumber() + "G4 P{}\n".format(SPINDLE_WAIT_TIME)
                SPINDLE_IS_STARTING = False

            if WAIT_FOR_SPINDLE and command == 'M3':
                out += linenumber() + "(enable spindle)\n"
                SPINDLE_IS_STARTING = True
